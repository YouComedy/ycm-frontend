...
if(!APP || !APP.ItemsViewer || !APP.ItemsViewer.Records ) {
...
Grid._prequestForm = function(data){
	$("#regtitle").text("Зарегистрируйтесь")
	$("#regreg").unblock().slider(1);
}
...
APP.on("change:" + STEP_KEY, function(){


	if(APP.get(STEP_KEY) == 1 && !$.cookie('YCM_SUBSCRIPTION')){
		...
		div.html([
			...
			'<form data-noreset="1" data-before="_beforeSubmit" data-owner="ItemsViewer" data-success="_prequestForm" style="margin:0" action="/register/submitPreTags" class="regform" method="post">',
				...
				'<div style="margin:0" class="categories">',
					...
					'</div>',
				'</div>',
				'<div class="" style="text-align: center;width: 100% !important;"><button class="button _xlarge">Зарегистрируйте меня</button></div>',
			'</form>',
		...
		div = $(document.createElement("div"));
		div.addClass("item panel panelSubscribeEmail");
		div.html([
			'<form action="/subscribe">',
				'<style>',
				'.panelSubscribeEmail form {',
					'margin: 0',
				'}',
				'.panelSubscribeEmail {',
					'padding: 20px;',
					'background: #ebf8ff;',
				'}',
				'.panelSubscribeEmail label {',
					'margin-left: 10px;',
				'}',
				'.panelSubscribeEmail label > input {',
					'margin: 0 5px;',
					'vertical-align: middle;',
				'}',
				'.panelSubscribeEmail__text {',
					'margin-right: 20px;',
					'font-weight: bold;',
				'}',
				'.panelSubscribeEmail ::-webkit-input-placeholder {',
					'color: #a7e0fd;',
				'}',
				'.panelSubscribeEmail :-moz-placeholder {',
					'color: #a7e0fd;',
				'}',
				'.panelSubscribeEmail ::-moz-placeholder {',
					'color: #a7e0fd;',
				'}',
				'.panelSubscribeEmail :-ms-input-placeholder {',
					'color: #a7e0fd;',
				'}',
				'.panelSubscribeEmail .tagsPanel__input {',
					'border-color: #a7e0fd;',
					'height: 50px;',
					'padding: 15px;',
					'font-size: 20px;',
				'}',
				'.panelSubscribeEmail ._xlarge {',
					'margin: 0;',
				'}',
				'</style>',
				'<h3 class="section link">Хотите получать подборку лучших шуток прямо на почту?</h3>',
				'<div class="section">',
					'<span class="panelSubscribeEmail__text">Выберите дни:</span>',
					'<label>',
						'Пн&nbsp;<input type="checkbox" name="summary_1" checked>',
					'</label>',
					'<label>',
						'Вт&nbsp;<input type="checkbox" name="summary_2">',
					'</label>',
					'<label>',
						'Ср&nbsp;<input type="checkbox" name="summary_3" checked>',
					'</label>',
					'<label>',
						'Чт&nbsp;<input type="checkbox" name="summary_4">',
					'</label>',
					'<label>',
						'Пт&nbsp;<input type="checkbox" name="summary_5" checked>',
					'</label>',
					'<label>',
						'Сб&nbsp;<input type="checkbox" name="summary_6">',
					'</label>',
					'<label>',
						'Вс&nbsp;<input type="checkbox" name="summary_7">',
					'</label>',
				'</div>',
				'<div class="section">',
					'<input class="button _xlarge _right" type="submit" value="Готово!">',
					'<input name="email" class="tagsPanel__input" placeholder="email@example.com">',
				'</div>',
			'</form>'
		].join(''));
		...
		var items = $('.item');
		var len = items.size();
		var pos = Math.floor(Math.random() * (len - 2)) + 1;
		items.eq(pos).after(div);
		...
		div.find('form').on('submit', function(e){
			...
			$.post(form.attr('action'), form.serialize(), function(data){
				...
				if( data && data.state === true ) {
					form.parent().fadeOut();
					$.alertOk('Вы успешно подписались на рассылку!');
					$.cookie('YCM_SUBSCRIPTION', true);
				} else {
					$.alertError((data && data['error_text']) || 'Ошибка отправки запроса');
				}
			});
		});

		/*div.find(".auth-service.vkontakte a").on("click", function(){Grid._dataFromRegForm = true}).eauth({"popup":{"width":585,"height":350},"id":"vkontakte"});
		div.find(".auth-service.facebook a").on("click", function(){Grid._dataFromRegForm = true}).eauth({"popup":{"width":585,"height":290},"id":"facebook"});*/
	}


	// if(APP.get(STEP_KEY) === 3) {
	// 	$(".item:last").prev().prev().before( div.clone() );
	// }
...
Items.on("renderSuccess", function(){
	APP.set(STEP_KEY, APP.get(STEP_KEY) + 1);
});
