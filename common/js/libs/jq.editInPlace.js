...
$.fn.editInPlace.defaults = {
	...
	bg_over:			undefined, // string: background color of hover of unactivated editor
	bg_out:				undefined, // string: background color on restore from hover
...
$.extend(InlineEditor.prototype, {
	...
	openEditor: function(anEvent) {
		if ( ! this.shouldOpenEditor(anEvent) || $(anEvent.target).is("a[class*=tag]"))
			return;
	...
	saveOriginalValue: function() {
		this.originalHTMLValue = this.dom.html();
		console.log(this.originalHTMLValue)
	...
	restoreOriginalValue: function() {
		// alert([111, this.originalHTMLValue])
		this.setClosedEditorContent(this.originalHTMLValue);
			// this.originalValue);
	},
	...
	connectClosingEventsToEditor: function() {
		...
		var textarea = form.find(".inplace_field");


		this.settings.on_focus && textarea.on('focus', this.settings.on_focus);
		this.settings.on_keyup && textarea.on('keyup', this.settings.on_keyup);
	...
	handleCancelEditor: function(anEvent) {
		...
		if (hasContent(enteredText)
			&& ! this.isDisabledDefaultSelectChoice()) {
					if(enteredText == this.originalValue )
						this.setClosedEditorContent(this.originalHTMLValue);
					else
	...
	isUnchangedInput: function(enteredText) {
		// alert([this.originalValue , enteredText])
	...
	handleSubmitToServer: function(enteredText) {
		var data = this.settings.update_value + '=' + encodeURIComponent(enteredText)
			...
			// + '&description' + this.settings.original_value + '=' + encodeURIComponent(this.originalValue);
